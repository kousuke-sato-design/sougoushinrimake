-- ============================================
-- MAツール 統合データベーススキーマ（最新版）
-- Supabase PostgreSQL
-- このファイルは現在のデータベース構造の完全なスナップショットです
-- マイグレーション履歴は supabase/migrations/ を参照してください
-- ============================================
...
CREATE INDEX idx_landing_pages_lp_type ON landing_pages(lp_type);
...
CREATE INDEX idx_lp_templates_type ON lp_templates(type);
CREATE INDEX idx_lp_templates_is_default ON lp_templates(is_default);
...
CREATE INDEX idx_customers_source_lp_id ON customers(source_lp_id);
...
-- 8. user_api_keys (ユーザーAPIキー管理 - 複数キー対応)
...
CREATE INDEX idx_user_api_keys_is_active ON user_api_keys(user_id, is_active);
...
CREATE INDEX idx_ai_generation_logs_lp_id ON ai_generation_logs(lp_id);
...
CREATE INDEX idx_lp_edit_history_landing_page_id ON lp_edit_history(landing_page_id);
CREATE INDEX idx_lp_edit_history_created_at ON lp_edit_history(created_at DESC);
...
CREATE INDEX idx_ai_conversation_history_landing_page_id ON ai_conversation_history(landing_page_id);
...
-- Storage設定
-- ============================================
-- Supabase Storageでlogosバケット作成
-- バケット名: logos
-- 公開設定: public
-- 許可形式: image/png, image/jpeg, image/jpg, image/svg+xml, image/webp
-- 最大サイズ: 5MB
